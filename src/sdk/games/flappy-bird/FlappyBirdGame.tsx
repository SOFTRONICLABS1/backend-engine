...
const SCREEN_SHAKE_DURATION = 500 // milliseconds
...
interface Particle {
  ...
  color: string
  size: number
  type: 'spark' | 'explosion' | 'trail' | 'celebration'
}
...
export const FlappyBirdGame: React.FC<FlappyBirdGameProps> = ({ notes }) => {
  ...
  const getParticleColor = (type: Particle['type']): string => {
    switch (type) {
      ...
      case 'trail': return '#4ECDC4'
      case 'celebration': return '#FF9F43'
      default: return '#FFFFFF'
    }
  }
  
  // Screen shake functions
  const triggerScreenShake = useCallback((intensity: number = SCREEN_SHAKE_INTENSITY, duration: number = SCREEN_SHAKE_DURATION) => {
    setScreenShake({
  ...
  const triggerVisualFeedback = useCallback((type: 'glow' | 'pulse' | 'correctPitch', duration: number = 200) => {
    setVisualFeedback(prev => ({ ...prev, [type]: true }))
    setTimeout(() => {
      setVisualFeedback(prev => ({ ...prev, [type]: false }))
    }, duration)
  }, [])
    ...
    const gameLoop = () => {
      ...
      setBird(prev => {
        ...
        if (isPitchDetectedRef.current && pitch > 0) {
          ...
          // Use consistent transition speed regardless of BPM - bird responsiveness should be constant
          const transitionSpeed = 0.4 // Fixed transition speed for consistent bird control
      ...
      if (gameState === 'playing') {
        ...
        const updatedPipes = prev.map(pipe => {
          // Track accuracy while bird is in pipe area (throttled to reduce performance impact)
          if (bird.x + BIRD_SIZE > pipe.x && bird.x < pipe.x + pipe.width && pitch > 0) {
            ...
            // Throttle accuracy sampling to every 3rd frame to improve performance
            if (accuracySamples.current.length === 0 || now - accuracySamples.current[accuracySamples.current.length - 1].timestamp > 50) {
              // Add current pitch sample (max once per 50ms)
  ...
  */
  ...
  return (
    <View style={[styles.container, {
      ...
      {/* Game UI overlay - Score left, Cycle right */}
      {(gameState === 'playing' || gameState === 'dying') && (
